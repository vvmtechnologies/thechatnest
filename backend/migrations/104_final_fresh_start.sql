-- Migration 104: Final fresh-start hard reset
--
-- Goal: leave the workspace with EXACTLY ONE user (support@vvmtechnologies.com)
-- in EXACTLY ONE organization. Wipes the second user (cs@alltoolhub.com) and
-- their ALLTOOLHUB organization that was created via fresh signup between
-- earlier resets.
--
...
BEGIN
  ...
  IF owner_user_id IS NULL THEN
    RAISE NOTICE 'Owner % not found — aborting fresh start.', owner_email;
    RETURN;
  END IF;

  -- 2. Owner's organization (the one we'll preserve)
  ...
  RAISE NOTICE 'Fresh start. Owner=% (user_id=%) org_id=%.',
    owner_email, owner_user_id, owner_org_id;

  -- 3. Wipe every tenant / operational table.
  --    Order doesn't matter for these — TRUNCATE CASCADE handles all FK chains.
  ...
  -- 4. Re-point the surviving organization's owner FK to our owner so
  --    fk_org_owner (ON DELETE RESTRICT) doesn't block the user delete.
  IF owner_org_id IS NOT NULL THEN
    UPDATE public.organizations SET owner_id = owner_user_id
    WHERE organization_id = owner_org_id;
  END IF;

  -- 5. Drop memberships → non-owner orgs → non-owner users
  ...
  -- 6. Re-affirm owner credentials so login keeps working
  UPDATE public.users
  ...
  RAISE NOTICE 'Fresh start complete. Only owner % remains.', owner_email;
END $$;
